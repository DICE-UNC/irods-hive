...
public class KEAFilter extends Filter implements OptionHandler {
	...
	public boolean input(Instance instance, Vocabulary vocabulary) throws Exception {
		...
		if (m_Dictionary == null) {
			...
			FastVector vector = convertInstance(instance, false, vocabulary);
	...
	public boolean batchFinished(Vocabulary vocabulary) throws Exception {
		...
		if (m_Dictionary == null) {
			buildGlobalDictionaries(vocabulary);
			buildClassifier(vocabulary);
			convertPendingInstances(vocabulary);
		}
		flushInput();
		m_NewBatch = true;
	...
	public void buildGlobalDictionaries(Vocabulary vocabulary) throws Exception {
		...
		for (int i = 0; i < getInputFormat().numInstances(); i++) {
			...
			HashMap hash = getPhrasesForDictionary(str, vocabulary);
		...
		for (int i = 0; i < getInputFormat().numInstances(); i++) {
			...
			while (it.hasNext()) {
				...
				double[] vals = featVals(phrase, phraseInfo, true,
						hashKeysEval, hashKeyphrases, length, hash, vocabulary);
	...
	private double[] featVals(String id, FastVector phraseInfo,
		...
		if (m_STDEVfeature) {
			 ...
			 * null) { //System.err.println(m_Vocabulary.getOrig(id) +
			 * " is a composite!"); newInst[m_STDEVIndex] = 1.0; } else {
		...
		if (m_NODEfeature) {
			...
			Vector idsRT = (Vector) vocabulary.getRelated(id);
		...
		if (m_LENGTHfeature) {
			...
			if (m_vocabulary.equals("none")) {
				...
				original = vocabulary.getOrig(id);
			}
			if (original == null) {
	...
	private void convertPendingInstances(Vocabulary vocabulary) throws Exception {
		...
		for (int i = 0; i < getInputFormat().numInstances(); i++) {
			Instance current = getInputFormat().instance(i);
			FastVector vector = convertInstance(current, true, vocabulary);
			Enumeration en = vector.elements();
			while (en.hasMoreElements()) {
				Instance inst = (Instance) en.nextElement();
				push(inst);
	...
	private FastVector convertInstance(Instance instance, boolean training, Vocabulary vocabulary)
		...
		if (!instance.isMissing(m_KeyphrasesAtt)) {
			...
			hashKeyphrases = getGivenKeyphrases(keyphrases, false, vocabulary);
			hashKeysEval = getGivenKeyphrases(keyphrases, true, vocabulary);
		}
		...
		while (it.hasNext()) {
			...
			double[] vals = featVals(id, phraseInfo, training, hashKeysEval,
					hashKeyphrases, length, hash, vocabulary);
	 ...
	 * features: vec = new FastVector(2);
	 * 
	 * // Update hashtable with all the info vec.addElement(new Counter(0)); //0
