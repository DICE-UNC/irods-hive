...
public class MauiTopicExtractor implements OptionHandler {
	...
	public void setOptions(final String[] options) throws Exception {
		...
		buildGlobalDictionary = Utils.getFlag('b', options);
		printGraph = Utils.getFlag('p', options);
		additionalInfo = Utils.getFlag('a', options);
	...
	public String[] getOptions() {
		...
		options[current++] = "-n";
		options[current++] = "" + (topicsPerDocument);
		options[current++] = "-t";
	 ...
	 */
	public void extractKeyphrases(final HashSet<String> fileNames)
			throws Exception {
		...
		for (String fileName : fileNames) {
			...
			for (int i = 0; i < topicsPerDocument; i++) {
				if (topRankedInstances[i] != null) {
					...
					if (printer != null) {
						 ...
						 * wikipedia.getMostLikelyArticle(topic, new
						 * CaseFolder()); } if (article != null) { if (root ==
						 * "") { root = article.getTitle(); }
						 * topics.put(article, new Integer(p)); } else { if
						 * (debugMode) {
						 * System.err.println("Couldn't find article for " +
						 * topic + " in " + documentTopicsFile); } } p++; }
						 */
						if (additionalInfo) {
							printer.print("\t");
							 ...
							 * searcher.searchConceptByKeyword(term); if
							 * (concepts.size() > 0) { term =
							 * concepts.get(0).getQName(); printer.print(term);
							 * }
							 */
			...
			/*
			 * if (printGraph) { String graphFile =
			 * documentTopicsFile.getAbsolutePath().replace(".key",".gv");
	 ...
	 * String outputFile) { FileOutputStream out; PrintWriter printer; try {
	 * 
	 * if (debugMode) { System.err.println("Printing graph information into " +
	 * outputFile); }
	 * 
	 * out = new FileOutputStream(outputFile); printer = new PrintWriter(out);
	 * 
	 ...
	 * 
	 * HashSet<String> done = new HashSet<String>(); double relatedness = 0; for
	 * (Article a : topics.keySet()) { int count = topics.get(a).intValue(); if
	 * (count < 1) { printer.print("\"" + a.getTitle() + "\" [fontsize=22];\n");
	 * } else if (count < 3) { printer .print("\"" + a.getTitle() +
	 * "\" [fontsize = 18];\n"); } else if (count < 6) { printer .print("\"" +
	 * a.getTitle() + "\" [fontsize = 14];\n"); } else { printer .print("\"" +
	 * a.getTitle() + "\" [fontsize = 12];\n"); }
	 * 
	 * for (Article c : topics.keySet()) { if (!c.equals(a)) { try { relatedness
	 * = a.getRelatednessTo(c); String relation = "\"" + a.getTitle() +
	 * "\" -- \"" + c.getTitle(); String relation2 = "\"" + c.getTitle() +
	 * "\" -- \"" + a.getTitle();
	 * 
	 * if (!done.contains(relation2) && !done.contains(relation)) {
	 * done.add(relation2); done.add(relation);
	 * 
	 * if (relatedness < 0.2) { printer.print(relation + "\"[style=invis];\n");
	 * } else { printer.print(relation + "\" [penwidth = \"" + (int)
	 * (relatedness * 10 - 0.2) + "\"];\n"); } } } catch (SQLException e) {
	 * e.printStackTrace(); } } } } printer.print("}\n"); printer.close();
	 * out.close(); } catch (Exception e1) { e1.printStackTrace(); } }
	 */
	...
	public void setInputDirectoryName(final String inputDirectoryName) {
	...
	public void setAdditionalInfo(final boolean additionalInfo) {
	...
	public void setBuildGlobalDictionary(final boolean buildGlobalDictionary) {
	...
	public void setVocabularyDirectory(final String vocabularyDirectory) {
	...
	public void setTopicsPerDocument(final int topicsPerDocument) {
