...
public class MARCtoSKOSConverter {
	...
	public void readInputFile(final String inputMARCfile,
			final PrintWriter skosOutputStream, final PrintWriter outputStream) {
		try {
	...
	public void buildPref2lccn(final String inputMARCfile) {
		try {
			InputStream in = new FileInputStream(inputMARCfile);
			MarcReader reader = new MarcStreamReader(in);
			...
			while (reader.hasNext()) {
				...
				while (i.hasNext()) {
					VariableField d = (VariableField) i.next();
					String tag = d.getTag();
					if (tag.equals("001") || tag.equals("150")
							|| tag.equals("151")) {
						processTag(tag);
					}
				}
	...
	public void processTag(final String tag) {
		String prefLabel = "";
		String altLabel = "";
		String rbterm = "";
		DataField dfield;
		...
		case 551:
			...
			while (vi.hasNext()) {
				...
				if (rbterm.length() > 0) {
					if (DEBUG) {
						System.out.println(rbterm);
					}
					if ((!rbterm.substring(0, 1).equals("g"))
							&& (!rbterm.substring(0, 1).equals("h"))) {
						lccn = pref2lccn.get(rbterm);
						if (lccn != null) {
							skos.addRelated(lccn);
						} else {
							nolccnout
									.println("No LCCN for related:  " + rbterm);
							noLCCNrelated++;
						}
					} else if (wfound && rbterm.substring(0, 1).equals("g")) { // $wg
																				// broader
						...
						if (lccn != null) {
							...
							nolccnout
									.println("No LCCN for broader:  " + rbterm);
							noLCCNbroader++;
						}
					} else if (wfound && rbterm.substring(0, 1).equals("h")) { // $wh
																				// narrower
						rbterm = rbterm.substring(3);
						lccn = pref2lccn.get(rbterm);
						if (lccn != null) {
							...
							nolccnout
									.println("No LCCN for narrower: " + rbterm);
							noLCCNnarrower++;
						}
					}
				}
	...
	public void writeSKOSConcept(final SKOSConcept skos,
			final PrintWriter outputStream) {
		outputStream.println("    <rdf:Description rdf:about=\"" + uri
				+ skos.getDescriptorId() + "#concept\">");
		outputStream.println("        <skos:prefLabel xml:lang=\"" + lang
				+ "\">" + skos.getPrefLabel() + "</skos:prefLabel>");
		if (!skos.getAltLabels().isEmpty()) {
			Iterator a = skos.getAltLabels().iterator();
			while (a.hasNext()) {
				outputStream.println("        <skos:altLabel xml:lang=\""
						+ lang + "\">" + a.next() + "</skos:altLabel>");
			}
		}
		if (!skos.getBroader().isEmpty()) {
			Iterator b = skos.getBroader().iterator();
			while (b.hasNext()) {
				outputStream.println("        <skos:broader rdf:resource=\""
						+ uri + b.next() + "#concept\"/>");
			}
		}
		if (!skos.getNarrower().isEmpty()) {
			...
			while (b.hasNext()) {
				outputStream.println("        <skos:narrower rdf:resource=\""
						+ uri + b.next() + "#concept\"/>");
			}
		}
		outputStream.println("        <skos:inScheme rdf:resource=\"" + uri
				+ "#conceptScheme\"/>");
		outputStream.println("        <skos:inScheme rdf:resource=\"" + uri
				+ "#" + skos.getConceptScheme() + "\"/>");
		outputStream
		...
		if (!skos.getRelated().isEmpty()) {
			...
			while (r.hasNext()) {
				outputStream.println("        <skos:related rdf:resource=\""
						+ uri + r.next() + "#concept\"/>");
			}
		}
		if (skos.getScopeNote() != "") {
			outputStream.println("        <skos:scopeNote xml:lang=\"" + lang
					+ "\">" + skos.getScopeNote() + "</skos:scopeNote>");
		}
		outputStream.println("    </rdf:Description>");
	}

	public void writeSKOSheader(final PrintWriter outputStream) {
		outputStream
	...
	public void writeSKOSConceptSchemes(final PrintWriter outputStream) {
		outputStream
				.println("    <rdf:Description rdf:about=\""
						+ uri
						+ "#conceptScheme\">\n"
	...
	public void writeSKOSfooter(final PrintWriter outputStream) {
